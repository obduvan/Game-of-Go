import copy
from game_move_log import GameMoveLog
from current_step_game import CurrentStepGame
from groups_move import GroupsMove
...
class Game:
    ...
    def move_is_valid(self, transformed_coord, normalize_coord, color):
        self.current_step_game = CurrentStepGame(transformed_coord, normalize_coord, copy.deepcopy(self.normalize_coord_stones_dict),
                                                 (self.black_groups), (self.white_groups), color)
        ...
        print(new_black_groups, "new bl")
        print(new_white_groups, "new wh ")

        self.validate_game_rule = ValidateGameRule(normalize_coord_stones_dict, new_black_groups, new_white_groups,
                                                   color)
    ...
    def set_new_move(self):
        ...
        self.normalize_coord_stones_dict = self.current_step_game.normalize_coord_stones_dict

        self.game_move_log.update_game_move_log(GroupsMove(self.black_groups, self.white_groups))
    ...
    def print_dict(self):
        for cord in self.normalize_coord_stones_dict:
            print(cord)
        print(self.black_groups)
        print(self.white_groups)
